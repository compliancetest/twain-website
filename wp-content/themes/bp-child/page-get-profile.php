...
if ($token) {
    ...
    if ($user && $password) {
        ...
        if ($password == $user_row->harness_password) {
            if ($meta_key) {
                $query = $wpdb->prepare("SELECT * FROM $wpdb->prefix" . "community_profile_instances cpi Right Join $wpdb->prefix" . "community_profile_meta cpm ON cpm.profile_id=cpi.id AND cpm.meta_key=%s AND cpm.meta_value=%s Order By cpm.meta_id desc, cpi.id desc, cpi.type desc Limit 1", $meta_key, $meta_value);
            } else {
