...
function getClaimsByProductId($product_id)
{
    ...
    $query = $wpdb->prepare("SELECT c.*, pm.meta_value as `issuer` FROM claims AS c LEFT JOIN " . $wpdb->postmeta . " as pm on pm.post_id=c.suite_minor_family_mark AND pm.meta_key='ts_issuer'  WHERE product_id=%d", $product_id);
