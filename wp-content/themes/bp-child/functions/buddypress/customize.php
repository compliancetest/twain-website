...
function bp_groups_admin_edit_mailchimp_list($item)
{
    ...
    foreach ($lists['data'] as $list) {
        ?><p><input type="radio" name="community_mailchimp_list_id" value="<?php echo $list['id'] ?>" <?php echo $list['id'] == $list_id ? 'checked="checked"' : '' ?> />
        <label><?php echo $list['name'] ?></label></p><?php
    }
    ?>
    <p>
        <a href="<?php echo admin_url() ?>?page=admin-actions&amp;admin-action=<?php echo wp_create_nonce('sync-users-to-mailchimp2') ?>&id=<?php echo $item->id ?>" target="_blank"
           class="button">Sync members with the selected list</a>
        ...
        if ($class == 'menu-communities') {
            ...
            if (count($groups) > 0) {
                foreach ($groups as $community) {
                    ...
                    if (doesUserCommunityAdmin(get_current_user_id(), $community->id) || doesUserCommunitySupport(get_current_user_id(), $community->id)) {
                        if(doesUserCommunityAdmin(get_current_user_id(), $community->id)) {
                            $item2[] = array('title' => 'Test Data Backups', 'url' => $community_url . 'backups');
                        }
                        $item2[] = array('title' => 'Settings', 'url' => $community_url . 'settings');
                    }
...
function cp_directory_groups_search_form()
{
    ...
    <form action="" method="get" id="search-groups-form">
        <label><input type="text" name="s" id="groups_search"
                      <?php if (!empty($_REQUEST['s'])){ ?>value="<?php echo esc_attr($search_value); ?>" <?php } else { ?> placeholder="<?php echo esc_attr($search_value) ?>" <?php } ?> /></label>
