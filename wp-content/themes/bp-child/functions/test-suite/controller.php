        ...
        if (!$_REQUEST['suite_id']) {
            ...
            if (!can_delete_suite($_REQUEST['suite_id'])) {
                ...
                if ($suites <= 1 && !$has_error) {
                    ...
                    $transactions = $wpdb->get_var($wpdb->prepare("SELECT count(id) FROM transactions WHERE test_suite_id  = %d", $suite->id));
                ...
                if (!$has_error) {
                    ...
                    if ($next_suite_id) {
                        ...
                        $wpdb->update($wpdb->prefix . "test_plans",
                            array('suite_id' => $next_suite_id),
                            array('suite_id' => $_REQUEST['suite_id']),
                            array('%d'),
                            array('%d')
...
//    $identifier = sanitize_title($_POST['ts_identifier']);
    ...
    if(DISPLAY_SUBSCRIPTIONS) {
        ...
        if($_POST['ts_tester_role'] == 'Application'){
            $planId = $wpdb->get_var("SELECT id FROM wp_pricing_plans WHERE id_str = 'TWAIN_APPLICATION'");
            update_post_meta($id, 'test_suite_plans', $planId);
        } else {
            $planId = $wpdb->get_var("SELECT id FROM wp_pricing_plans WHERE id_str = 'TWAIN_DATA_SOURCE'");
            update_post_meta($id, 'test_suite_plans', $planId);
        }
        update_post_meta($id, 'test_suite_plans_order', '');
    }
