...
require_once(THE_FUNCTION . '/subscription/class.charge.php');
...
function ct_add_manage_subscriptions_menu()
{
    ...
    add_submenu_page("manage-payments", "Add Charge Entry", "Add Charge Entry", "manage_options", "add-charge", "ct_add_charge");
    ...
    <div class="wrap">
        ...
        <form name="adminform" action="<?php echo admin_url()?>admin.php?page=add-charge<?php echo $id ? ('&id=' . $id) : ''?>" method="post">
            <table class="widefat" style="width: auto;">
                ...
                <tr>
                    <th>End Date</th>
                    <td><input type="text" name="end_date" id="end_date" value="<?php echo $data['end_date'] ? $data['end_date'] : date( 'Y-m-d', strtotime('+7 days') );?>" required="required"/></td>
                </tr>
                <tr>
    ...
    if(isset($_REQUEST['org-action']))
    {
        ...
        if($action == 'save-charge')
        {
            ...
            if( $resp )
            {
                ...
                <script type="text/javascript">
                    setTimeout(function(){
                        document.location.href = '<?php echo admin_url()?>admin.php?page=charges';
                    }, 1000);
                </script>
                <?php
