    ...
    if ($not_allowed) {
        $html .= '<div class="grid-cell width50P has-tooltip">
                    <input type="radio" disabled="disabled" name="' . $name . '" id="' . $name . $instance->id . '" value="' . $instance->id . '"' . cp_checked($instance->id, $defaults) . ' class="right10" />
                        <a href="' . get_site_url() . '?td-action=' . wp_create_nonce('view-profile-instance') . '&id=' . $instance->id . '&back=1" rel="custom-popup" cp-type="ajax">' . $instance->profile_name . '</a>
    ...
    if (!$suites) {
        ...
        <div class="popup-box" id="trigger-message-box" style="display: none; width: 555px;">
            ...
            <div class="popup-box-content grid-box-body">
                <p class="message warning">You need to purchase subscription for at least one test suite to trigger
                    message.</p>
            </div>
...
//            $profileInstances = _getTesterAndHarnessProfileInstances($current_case_id, $user_id);
            ...
            <div class="popup-box" id="trigger-message-box" style="display: none; width: 555px;">
                <form name="messageForm" id="messageForm" action="">
                    <div class="popup-box-header radius6 noradiusbottom">Trigger Message</div>
                    <div class="popup-box-content grid-box-body">
                        <p>Send a message from our test harness to your system. Choose a test case, then select the
                            template and, optionally change the default values for template variables</p>
                        ...
                        <div class="info-section">
                                ...
                                <div class="grid-cell width250 left15">
                                    ...
                                    <select name="product" id="tm-product" class="select">
                                        ...
                                        <?php foreach ($products as $p) { ?>
                                            <option
                                                value="<?php echo $p->ID ?>" <?php echo ($p->ID == $current_product_id || count($products) == 1) ? 'selected="selected"' : '' ?>><?php echo get_post_meta($p->ID, 'product_name', true) ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                                <div class="clear"></div>
                                    ...
                                    <select name="test-case" id="tm-test-case" class="select">
                                        ...
                                        <?php foreach ($cases AS $c):
                                            ...
                                            <option value="<?php echo $c->ID ?>"
                                                    data-bulk="<?php echo $cs->bulk == 'Yes' && PricingPlan::isSupportBulk($pricingPlanId) ? 'Yes' : 'No'; ?>"><?php echo $c->post_title ?></option>
                                        <?php endforeach ?>
                                    </select>
                                </div>
                                <div class="grid-cell width250 left15">
                                    ...
                                    <select name="template" id="tm-template" class="select">
                                        ...
                                        <?php foreach ($caseTemplates as $t) { ?>
                                            <option
                                                value="<?php echo $t['url'] ?>" <?php echo (count($caseTemplates) == 1) ? 'selected="selected"' : '' ?>><?php echo $t['name'] ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                                <div class="clear"></div>
                            ...
                            <div class="field-row">
                                <div class="grid-cell width250">
                                    <input type="checkbox" name="show_my"
                                           id="show_my" <?php if ($is_checked_by_default): ?> checked="checked"<?php endif; ?>>
                                ...
                                <div class="grid-cell width250 left15 copy_count_container displaynone">
                                    ...
                                    <input type="text" name="copy_count" id="copy_count" maxlength="4"
                                           class="input small-text" style="width: 50px;" value="1">
                                </div>
                            </div>
                            <div class="clear"></div>
                            <div class="space10"></div>
                    ...
                    <div class="popup-box-footer radius6 noradiustop">
                        <a href="#" class="action-btn process-btn has-tooltip" id="send-message-link"><span
                    ...
                    <div class="loading loading-with-text radius6">
                        <div><b>LOADING DATA</b><span>Please wait...</span></div>
                    </div>
                    <input type="hidden" name="ct-message-action"
                           value="<?php echo wp_create_nonce('send-message') ?>"/>
...
function uploadMessage()
{
    ...
    if (!$suite_id || !$case_id || !$file || $file['error'] != UPLOAD_ERR_OK) {
        ...
        if (!$subscription || !in_array($suite_id, $cases)) //Test Suite And Case Validation
        {
            ...
            $query = $wpdb->prepare("SELECT COUNT(*) FROM wp_users_transactions_files WHERE fileId LIKE %s", $fileId . "%");
            ...
            if (get_option('saff_upload_via_sqs') == 'yes') {
                $filecontent = '';
            }
            //Save Data To Transactions Files Table
            $return = $wpdb->insert("wp_users_transactions_files",
                array(
                    'fileId' => $fileId,
                    'fileName' => $filename,
                    'content' => $filecontent,
            ...
            if (get_option('saff_upload_via_sqs') == 'yes') {
                $sqs = new SqsWrapper(get_option('saff_upload_via_sqs_queue_name'));
                $message = array(
                    'operation' => 'inboundUploadMessageRequest',
                    'correlationID' => 'd4342fsc5-fa89-44f6-9286-c38a751dbac',
                    'securityContext' => array(
                        'username' => $subscription->harness_username
                    ),
                    'parameters' => array(
                        'message' => array(
                        ...
                        'productId' => get_post_meta($product_id, 'product_id', true)
                    )
                );
