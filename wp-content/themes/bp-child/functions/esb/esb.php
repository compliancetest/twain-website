...
//                    $orderQuery .= ' ORDER BY FROM_PARTY_ID ' . $order;
        ...
        if($ids)
        {
            $query = "SELECT
                        DISTINCT(m.ID),
                        m.ID, m.MSH_CONVERSATION_ID, m.MESSAGE_TIMESTAMP, m.S3_PAYLOAD_LOCATION, m.S3_PAYLOAD_CONTENT_LENGTH, m.FROM_PARTY_ID, m.TO_PARTY_ID, m.SERVICE, m.ACTION, m.MSH_MESSAGE_OUTCOME_STATUS_ID, m.MESSAGE_ID, m.COPY_COUNT, m.RESPONSE_STATUS, m.S3_RESPONSE_LOCATION,
