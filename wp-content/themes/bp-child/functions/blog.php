    ...
    <div style="border: solid 1px #ddd">
        <div class="tabs-panel" style="padding: 5px 10px">
            <ul class="categorychecklist form-no-clear" style="margin: 0;">
                <li><label class="selectit"><input type="radio" name="blog_community" value="0" <?php if (!$community_id){ ?>checked="checked" <?php } ?> /> Not assigned</label>
                </li>
...
function ct_get_blog_years($type = null, $community_id = null)
{
    ...
    if ($community_id) {
        $where[] = $wpdb->prepare(" pm.meta_value=%s ", $community_id);
    }
        ...
        <ul class="articles" id="<?php echo $postType ?>">
            ...
            foreach ($articles as $article) {
                if ($postType == 'event') {
                    ?>
                    <li>
                        <h5><a href="<?php echo get_permalink($article->ID) ?>"><?php echo get_the_title($article) ?></a> (<span
                                class="date"><?php echo get_post_meta($article->ID, 'event_date', true) ?></span>)</h5>
                    </li>
                    <?php
                    ...
                    ?>
                    <li>
                        <h5 class="left"><a href="<?php echo get_permalink($article->ID) ?>"><?php echo get_the_title($article) ?></a> <span
                                class="date">(<?php echo formatDate($article->post_date); ?>)</span></h5>
