            ...
            <div class="search-result-form">
                <div class="searchform">
                    <input type="text" name="q" id="q" class="keyword"
                           ...
                           placeholder="Enter a Product Name, Service Name or Business Name" autocomplete="off"/>
                                ...
                                <?php if (is_array($allFilters->getPath('facets/type/buckets'))): ?>
                                    <?php foreach ($allFilters->getPath('facets/type/buckets') AS $v): ?>
                                        ...
                                        <?php elseif ($v['value'] == 'Agreement'): ?>
                                            <option
                                                value="<?php echo $v['value']; ?>" <?php if (isset($_GET['type']) && $_GET['type'] == $v['value']): ?> selected="selected" <?php endif; ?>><?php echo 'Agreement'; ?></option>
                                        <?php else: ?>
                                            <option
                                                value="<?php echo $v['value']; ?>" <?php if (isset($_GET['type']) && $_GET['type'] == $v['value']): ?> selected="selected" <?php endif; ?>><?php echo 'Product'; ?></option>
                                        <?php endif; ?>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
        ...
        <?php if ($results['hits']['found'] > 0): ?>
            <div class="search-results-count clearfix">
                <p class="search-results-count-label">
                    Showing <?php echo $results->getPath('hits/start') + 1; ?>
                    - <?php echo $results->getPath('hits/start') + count($results->getPath('hits/hit')); ?> of
                    <b><?php echo $results->getPath('hits/found'); ?></b> Results
                ...
                <table class="search-result-list">
                    ...
                    <tr>
                        <th class="first">
                            ...
                            <div class="sorting-box">
                                <a href="<?php echo get_permalink() ?>?<?php echo implode("&", $params) ?>&orderby=name&order=asc"
                                   class="sorting-link asc<?php if ($orderby == 'name' && $order == 'asc') { ?> current<?php } ?>"><span
                                        class="sort"></span></a>
                                <a href="<?php echo get_permalink() ?>?<?php echo implode("&", $params) ?>&orderby=name&order=desc"
                                   class="sorting-link desc<?php if ($orderby == 'name' && $order == 'desc') { ?> current<?php } ?>"><span
                                        class="sort"></span></a>
                            </div>
                        </th>
                        <th>Version</th>
                        ...
                        <th>
                            ...
                            <div class="sorting-box">
                                <a href="<?php echo get_permalink() ?>?<?php echo implode("&", $params) ?>&orderby=date&order=asc"
                                   class="sorting-link asc<?php if ($orderby == 'date' && $order == 'asc') { ?> current<?php } ?>"><span
                                        class="sort"></span></a>
                                <a href="<?php echo get_permalink() ?>?<?php echo implode("&", $params) ?>&orderby=date&order=desc"
                                   class="sorting-link desc<?php if ($orderby == 'date' && $order == 'desc') { ?> current<?php } ?>"><span
                                        class="sort"></span></a>
                            </div>
                    ...
                    <?php if ($results->getPath('hits/found') > 0): ?>
                        <?php foreach ($results->getPath('hits/hit') AS $row): ?>
                            ...
                            <?php if ($row_data['type'][0] == 'Agreement'): ?>
                                ...
                                <tr>
                                    <td class="first">
                                        <div class="agreement-row"><a
                                                href="<?php echo get_permalink($agreement->requester_service_id); ?>"
                                                class="blue_txt"><?php echo $requester_service->service_name; ?></a>
                                        </div>
                                        <a href="<?php echo get_permalink($agreement->responder_service_id); ?>"
                                           class="blue_txt"><?php echo $responder_service->service_name; ?></a>
                                    </td>
                                    <td>
                                        <div
                                            class="agreement-row"><?php echo $requester_service->service_version; ?></div>
                                        <?php echo $responder_service->service_version; ?>
                                    </td>
                                    <td>
                                        <div
                                            class="agreement-row"><?php echo $requester_service->service_owner; ?></div>
                                        <?php echo $responder_service->service_owner; ?>
                                    </td>
                                    <td>Agreement</td>
                                    <td class="test-suite-column">
                                        <div class="agreement-row"><a
                                                href="<?php echo get_permalink($requester_service->service_suite_id); ?>"><?php echo get_the_title($requester_service->service_suite_id); ?></a>
                                        </div>
                                    ...
                                    <td>
                                        <div
                                            class="agreement-row"><?php if (!empty($requester_service->service_roles)) echo implode(', ', $requester_service->service_roles); ?></div>
                                        <?php if (!empty($responder_service->service_roles)) echo implode(', ', $responder_service->service_roles); ?>
                                    </td>
                                    <td>
                                        <div
                                            class="agreement-row"><?php if (!empty($requester_service->service_levels)) echo implode(', ', $requester_service->service_levels); ?></div>
                                    ...
                                    <?php if (is_super_admin()): ?>
                                        <td class="remove_entry"><a
                                                href="/?_psnonce=<?php echo wp_create_nonce('get-delete-search-entry'); ?>&id=<?php echo $row['id']; ?>"
                            ...
                            <?php else: ?>
                                <tr>
                                    ...
                                    <td class="test-suite-column"><a
                                            href="<?php echo get_permalink($row_data['suite_id'][0]); ?>"><?php echo $row_data['test_suite'][0]; ?></a>
                                    </td>
