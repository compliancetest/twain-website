...
function bp_docs_doc_action_links() {
	...
	$links[] = '<li><a href="' . bp_docs_get_doc_link() . '">' . __( '', 'bp-docs' ) . '
				<img src="'. get_bloginfo('stylesheet_directory') .'/images/view_doc.png" class="top3" />
				<span class="simple_tooltip radius6">Read<span></span></span>
				</a></li>';

	if ( bp_docs_current_user_can( 'edit', get_the_ID() ) ) {
		$links[] = '<li><a href="' . bp_docs_get_doc_link() . BP_DOCS_EDIT_SLUG . '" class="bold">' . __( '', 'bp-docs' ) . '
					<img src="'. get_bloginfo('stylesheet_directory') .'/images/edit_doc.png" class="top1"/>
					<span class="simple_tooltip radius6">Edit<span></span></span></a>
					</a></li>';
	}

	if ( bp_docs_current_user_can( 'view_history', get_the_ID() ) && defined( 'WP_POST_REVISIONS' ) && WP_POST_REVISIONS ) {
		$links[] = '<li><a href="' . bp_docs_get_doc_link() . BP_DOCS_HISTORY_SLUG . '" class="bold">' . __( '', 'bp-docs' ) . '
					<img src="'. get_bloginfo('stylesheet_directory') .'/images/history_doc.png" class="top1"/>
					<span class="simple_tooltip radius6">Hystory<span></span></span></a>
		</a></li>';
	}
...
function bp_docs_create_button() {
	if ( ! bp_docs_is_doc_create() && bp_docs_current_user_can( 'create' ) ) {
		echo apply_filters( 'bp_docs_create_button', '<a class="button button_medium red_bcg white_txt radius6 create_document" id="bp-create-doc-button" href="' . bp_docs_get_create_link() . '">' . __( "Create New Doc", 'bp-docs' ) . '</a>' );
	}
}
