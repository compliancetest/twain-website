...
function bp_groups_admin_load() {
	...
	if ( $doaction && 'save' == $doaction ) {
		...
		if ( ! empty( $_POST['terms_co'] ) ) {
			$terms= $_POST['terms_co'];
		    // die($terms);
			$terms_result = $wpdb->get_row( "SELECT * FROM " . $wpdb->prefix . "bp_groups_terms WHERE group_id={$_GET['gid']}");
			$terms_db = $terms_result->content;
			if (empty($terms_db)){
				$wpdb->insert( 
						$wpdb->prefix.'bp_groups_terms', 
						array( 
							'group_id' => $group_id, 
							'content' => $terms
						), 
...
function bp_groups_admin_edit() {
	...
	<div class="wrap">
		...
		<?php if ( ! empty( $group ) ) : ?>
			...
			<form action="<?php echo esc_attr( $form_url ); ?>" id="bp-groups-edit-form" method="post" enctype="multipart/form-data">
				<div id="poststuff">
...
function bp_groups_admin_edit_terms( $item ){
	global $wpdb;
...
function bp_groups_admin_edit_metabox_attachments( $item ){
	global $wpdb;
	$myrows = $wpdb->get_results( "SELECT * FROM " . $wpdb->prefix . "bp_groups_downloads WHERE group_id={$_GET['gid']}");
	foreach ($myrows as $key => $row){
		echo '<div class="elem-file">';
		echo $row->name;
	...
	<input type="file" name="attachment_group[]" > <br />
	<div class="clear"></div>
	</div> 
	<div class="copy-correct-file">	</div>
	 <a class="add_new_file button right ">Add New</a>
	 <br /> <br />
		...
		<div class="bp-groups-member-type" id="bp-groups-member-type-<?php echo esc_attr( $member_type ) ?>">
			...
			<h4>
				<?php switch ( $member_type ) :
					case 'admin'  : _e( 'Community Administrators', 'buddypress' ); break;
					...
					<tr>
						...
						<td class="urole-column">
							<select class="bp-groups-role" id="bp-groups-role-<?php echo esc_attr( $type_user->user_id ); ?>" name="bp-groups-role[<?php echo esc_attr( $type_user->user_id ); ?>]">
								<option value="admin" <?php selected( 'admin', $member_type ) ?>><?php _e( 'Community Administrator', 'buddypress' ) ?></option>
