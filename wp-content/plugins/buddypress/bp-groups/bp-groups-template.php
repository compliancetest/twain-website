	...
	function bp_get_group_join_button( $group = false ) {
		...
		if ( isset( $group->is_member ) && $group->is_member ) {
			...
			$button = array(
				...
				'link_class'        => 'group-button leave-group group-button request-membership button button_medium red_bcg white_txt radius6 group-button pending membership-requested',
			);
				...
				case 'private' :
					...
					if ( !bp_group_has_requested_membership( $group ) ) {
						$button = array(
							...
							'link_text'         => __( 'Join Community', 'buddypress' ),
							'link_title'        => __( 'Request Membership', 'buddypress' ),
							'link_class'        => 'group-button request-membership button button_medium red_bcg white_txt radius6 group-button pending membership-requested',
						);
					...
					// Member has requested membership already
						...
						$button = array(
							...
							'link_class'        => 'group-button pending membership-requested button button_medium red_bcg white_txt radius6 group-button pending membership-requested',
						);
					}
