        ...
        <div class="form-group">
            ...
            <select name="product_id" id="availableProducts" class="form-control">
                ...
                @foreach($products as $product)
                    <option value="{{ $product->ID }}" @if($product->ID == $testPlan->product_id) selected="selected" @endif>{{ $product->post_title . ' V' .getPostMeta($product->ID, 'product_version') }}</option>
                @endforeach
            </select>
        </div>
        <div class="row">
            ...
            <div class="col-sm-6">
                ...
                @foreach($roles as $role)
                    <div class="radio">
                        <label>
                            <input type="radio" name="role" class="role" value="{{ str_replace(' ', '', $role) }}" @if(str_replace(' ', '', $role) == str_replace(' ', '', $testPlan->role)) checked="checked" @endif>
                            {{ $role }}
                        </label>
                    </div>
                @endforeach
