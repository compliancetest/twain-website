...
class TestSuitesController extends Controller
{
    ...
    public function view($testSuiteSlug, Request $request)
    {
        ...
        if (empty($request->toArray()) && count($testSuite->getCases([], $isAdmin, true)->get())) {
    ...
    public function destroy($testSuiteSlug)
    {
        ...
        if (count($testSuite->subscribers()->where('user_id', '!=', 0)->get())) {
            $messages[] = "The test suite can't be deleted because " . count(count($testSuite->subscribers()->where('user_id', '!=', 0)->get())) . " subscriptions still reference it.";
        }
